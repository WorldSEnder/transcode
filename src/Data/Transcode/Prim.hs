...
fixedSizeStorable :: forall m n s. (Writable m, Readable n, MonadDecodeDiagnostics n, F.Storable s) => Transcode m n s s s
fixedSizeStorable = transcoder encode decode where
  encode :: s -> m s
    -- to guarantee the laws are not violated, we could reread the s written. We assume this is not necessary
  encode s = s <$ writeBuilder (BP.primFixed BPI.storableToF s)
  decode :: n (s, s)
  decode = do
    (BI.BS ptr len) <- splitStrict size
    if len < size
    then R.fatal R.UnexpectedEof
    else let s = peekOne ptr in P.pure (s, s)
...
word8Host :: (Writable m, Readable n, MonadDecodeDiagnostics n) => Transcode m n Word8 Word8 Word8
word8Host = fixedSizeStorable
word16Host :: (Writable m, Readable n, MonadDecodeDiagnostics n) => Transcode m n Word16 Word16 Word16
word16Host = fixedSizeStorable
word32Host :: (Writable m, Readable n, MonadDecodeDiagnostics n) => Transcode m n Word32 Word32 Word32
